...
sed -i 's!-Wunused-but-set-variable!!g' Makefile.*
sed -i 's!-Wclobbered!!g' Makefile.*

# Create tmpfiles.d config.
%{__mkdir} -p omv_addons
%{__cat} << EOF > omv_addons/%{name}.conf
d %{_rundir}/%{name} 0755 root root -
EOF

# Create desktop-file for the monitor-app.
%{__cat} << EOF > omv_addons/%{name}-monitor.desktop
[Desktop Entry]
Name=%{name} Monitor
...
%{__cat} << EOF > omv_addons/%{name}-monitor.svg
...
%{__cat} << EOF > omv_addons/%{name}-monitor.ReadMe.txt
...
%build
CXXFLAGS="%{optflags} -std=c++11"%configure									\
...
# Setup tmpfiles.d
%{__install} -Dpm 0644 omv_addons/%{name}.conf				\
...
%{__install} -Dpm 0755 tools/thermal_monitor/ThermalMonitor	\
	%{buildroot}%{_bindir}/ThermalMonitor
%{__install} -Dpm 0644 omv_addons/%{name}-monitor.desktop			\
	%{buildroot}%{_datadir}/applications/%{name}-monitor.desktop
%{__install} -Dpm 0644 omv_addons/%{name}-monitor.svg			\
